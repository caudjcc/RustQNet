    ...
    def __init__(self, config, **kwargs):
        ...
        if config.DATASET.RGB_modal:
            ...
            if self.count_true>1 and self.MI_cul:
                self.convx1_rgb = nn.Conv2d(in_channels=self.in_channels[0], out_channels=channel, kernel_size=3, padding=1)
                self.convx2_rgb = nn.Conv2d(in_channels=self.in_channels[1], out_channels=channel, kernel_size=3, padding=1)
                self.convx3_rgb = nn.Conv2d(in_channels=self.in_channels[2], out_channels=channel, kernel_size=3, padding=1)
                self.convx4_rgb = nn.Conv2d(in_channels=self.in_channels[3], out_channels=channel, kernel_size=3, padding=1)
        ...
        if config.DATASET.MUL_modal:
            ...
            if self.count_true>1 and self.MI_cul:
                self.convx1_mul = nn.Conv2d(in_channels=self.in_channels[0], out_channels=channel, kernel_size=3, padding=1)
                self.convx2_mul = nn.Conv2d(in_channels=self.in_channels[1], out_channels=channel, kernel_size=3, padding=1)
                self.convx3_mul = nn.Conv2d(in_channels=self.in_channels[2], out_channels=channel, kernel_size=3, padding=1)
                self.convx4_mul = nn.Conv2d(in_channels=self.in_channels[3], out_channels=channel, kernel_size=3, padding=1)
        if config.DATASET.VI_modal:
            ...
            if self.count_true>1 and self.MI_cul:
                self.convx1_vi = nn.Conv2d(in_channels=self.in_channels[0], out_channels=channel, kernel_size=3, padding=1)
                self.convx2_vi = nn.Conv2d(in_channels=self.in_channels[1], out_channels=channel, kernel_size=3, padding=1)
                self.convx3_vi = nn.Conv2d(in_channels=self.in_channels[2], out_channels=channel, kernel_size=3, padding=1)
                self.convx4_vi = nn.Conv2d(in_channels=self.in_channels[3], out_channels=channel, kernel_size=3, padding=1)
        ...
        if self.count_true == 1:
            self.conv_concat = BasicConv2d(sum(self.in_channels) , last_inp_channels, kernel_size=3, padding=1)
        if self.count_true == 2:
            self.conv_concat = BasicConv2d(sum(self.in_channels) * 2, last_inp_channels, kernel_size=3, padding=1)
            if self.MI_cul:
