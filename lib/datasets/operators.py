...
class Transform(object):
    ...
    def apply(self, sample):
        if 'image' in sample:
            ...
            sample['image3'] = self.apply_im(sample['image_t3'])
        if 'mask' in sample:
...
class DecodeImg(Transform):
        ...
        if img_format == 'tiff' or ext == '.img':
            try:
                ...
                try:
                    ...
                    raise ImportError( "Failed to import gdal! Please install GDAL library according to the document.")
            dataset = gdal.Open(img_path)
            if dataset == None:
                raise IOError('Cannot open', img_path)
            im_data = dataset.ReadAsArray()
        elif img_format in ['jpeg', 'bmp', 'png', 'jpg']:
            im_data = np.asarray(Image.open(img_path))
        elif ext == '.npy':
            im_data = np.load(img_path)
        else:
            raise TypeError("Image format {} is not supported!".format(ext))
    ...
    def apply(self, sample):
        ...
        if 'image3' in sample:
            if self.read_geo_info:
                image3, geo_info_dict3 = self.apply_im(sample['image3'])
                sample['image3'] = image3
                sample['geo_info_dict3'] = geo_info_dict3
            else:
                sample['image3'] = self.apply_im(sample['image3'])
