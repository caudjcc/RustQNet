...
import random
#from .operators import DecodeImg, Resize, RandomHorizontalFlip, Normalize, Compose, ArrangeSegmenter, ReloadMask

class mydataset_cls(BaseDataset):
    def __init__(self, 
                 root, 
                 list_path, 
                 num_samples=None, 
        ...
        super(mydataset_cls, self).__init__(ignore_label, base_size,
    ...
    def convert_label_9_cls(self, mask):#9类
        mask[(mask >= 0.82) & (mask < 1)] = 8  # DI0.8~1
        mask[(mask >= 0.64) & (mask < 1)] = 7  # DI0.6~0.8
        mask[(mask >= 0.46) & (mask < 1)] = 6  # DI0.4~0.6
        mask[(mask >= 0.28) & (mask < 1)] = 5  # DI0.2~0.4
        mask[(mask >= 0.19) & (mask < 1)] = 4  # DI0.1~0.2
        mask[(mask >= 0.145) & (mask < 1)] = 3  # DI0.05~0.1
        mask[(mask > 0.1) & (mask < 1)] = 2  # DI0.001~0.05
        mask[(mask == 0.1) & (mask < 1)] = 1  # health
        mask[mask < 0.1] = 0#solid
        if isinstance(mask, np.ndarray):
            return mask.astype(int)  # 将mask转换为整数类型
        elif isinstance(mask, torch.Tensor):
            return mask.to(torch.int)  # 将mask转换为整数类型
        else:  raise IOError('Cannot support', mask.type())
    
    def convert_label_7_cls(self, mask):#7类
        mask[(mask > 0.37) & (mask < 1)] = 6  # DI0.4~0.6
        mask[(mask > 0.28) & (mask < 1)] = 5  # DI0.2~0.4
        mask[(mask > 0.19) & (mask < 1)] = 4  # DI0.1~0.2
        mask[(mask > 0.145) & (mask < 1)] = 3  # DI0.05~0.1
        mask[(mask > 0.1) & (mask < 1)] = 2  # DI0.001~0.05
    ...
    def convert_label_5_cls(self, mask):#5类
        mask[(mask > 0.37) & (mask < 1)] = 4  # DI0.4~0.6
        mask[(mask > 0.145) & (mask < 1)] = 3  # DI0.05~0.1
        mask[(mask > 0.1) & (mask < 1)] = 2  # DI0.001~0.05
        mask[(mask == 0.1) & (mask < 1)] = 1  # health
        mask[mask < 0.1] = 0#solid
    ...
    def convert_label_4_cls(self, mask):#5类
        mask[(mask > 0.46)] = 3 # DI0.001~0.05
        mask[(mask > 0.19) & (mask <1)] = 2  # DI0.001~0.05
        mask[(mask >= 0.1) & (mask <1)] = 1  # health
        mask[mask < 0.1] = 0#solid
        if isinstance(mask, np.ndarray):
            return mask.astype(int)  # 将mask转换为整数类型
    ...
    def convert_label_3_cls(self, mask):#5类
        mask[(mask > 0.28)] = 2  # DI0.001~0.05
        mask[(mask >= 0.1) & (mask <1)] = 1  # health
        mask[mask < 0.1] = 0#solid
        if isinstance(mask, np.ndarray):
            return mask.astype(int)  # 将mask转换为整数类型
    ...
    def read_img(self, path):        
        ...
        if im_data.ndim == 2:
            #im_data = F.to_intensity(im_data)#将图像数据转换为强度值（在0到255范围内）。
    ...
    def __getitem__(self, index):
        ...
        label = self.convert_label_9_cls(label)
...
def cv_random_flip(RGB, label, MUL, VI):  # accept tensor data instead of PIL images
    if torch.rand(1) > 0.5:
        ...
        if flip_flags[0] == 1:
            RGB = torch.flip(RGB,
                             [-1])  # Use torch.flip() to perform the left-right flipping operation on the tensor data.

            label = torch.flip(label, [
                -1])  # The [-1] argument indicates the last dimension, which corresponds to flipping along the horizontal axis.
        ...
        if flip_flags[1] == 1:
            RGB = torch.flip(RGB,
                             [-2])  # Use torch.flip() to perform the left-right flipping operation on the tensor data.

            label = torch.flip(label, [
                -2])  # The [-1] argument indicates the last dimension, which corresponds to flipping along the horizontal axis.
...
def MULti_scale(RGB, label, MUL, VI):
    ...
    if torch.rand(1) > 0.5:
        ...
        transformations_MUL = []
        if scale < 1.0:
            resize_transform = transforms.Resize(size=(int(target_size[0] * scale), int(target_size[1] * scale)),
                                                 interpolation=Image.BICUBIC)
            pad_transform = transforms.Pad(padding=(int((target_size[1] - int(target_size[1] * scale)) / 2),
                                                    int((target_size[0] - int(target_size[0] * scale)) / 2)), fill=0)
            transformations_MUL.append(transforms.Compose([resize_transform, pad_transform]))

            resize_transform = transforms.Resize(
                size=(int(2 * target_size[0] * scale), int(2 * target_size[1] * scale)), interpolation=Image.BICUBIC)
            pad_transform = transforms.Pad(padding=(int((2 * target_size[1] - int(2 * target_size[1] * scale)) / 2),
                                                    int((2 * target_size[0] - int(2 * target_size[0] * scale)) / 2)),
                                           fill=0)
...
def randomRotation(RGB, label, MUL, VI):
    if torch.rand(1) > 0.5:
        ...
        RGB = TF.rotate(RGB,
                        random_angle)  # Set resample=False to disable resampling and maintain the original pixel values.

        label = TF.rotate(label,
                          random_angle)  # https://pytorch.org/vision/stable/generated/torchvision.transforms.functional.rotate.html
...
def colorEnhance(image):  # expected to be in […, 1 or 3, H, W] format
    if torch.rand(1) > 0.8:
        bright_intensity = torch.randint(5, 15, (
        1,)).item() / 10.0  # 0 gives a black image, 1 gives the original image while 2 increases the brightness by a factor of 2.
        image = TF.adjust_brightness(image, bright_intensity)
        contrast_intensity = torch.randint(5, 15, (
        1,)).item() / 10.0  # 0 gives a solid gray image, 1 gives the original image while 2 increases the contrast by a factor of 2
        image = TF.adjust_contrast(image, contrast_intensity)
        color_intensity = torch.randint(5, 15, (
        1,)).item() / 10.0  # 0 will give a black and white image, 1 will give the original image while 2 will enhance the saturation by a factor of 2.
        image = TF.adjust_saturation(image, color_intensity)
        sharp_intensity = torch.randint(5, 15, (1,)).item() / 10.0
        image = TF.adjust_sharpness(image,
                                    sharp_intensity)  # 0 gives a blurred image, 1 gives the original image while 2 increases the sharpness by a factor of 2.
    return image
