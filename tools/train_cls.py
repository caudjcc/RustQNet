...
from core.criterion import OhemCrossEntropy
from core.function_cls import train, validate
...
def parse_args():
    ...
    parser.add_argument('--cfg',
                        ...
                        default=r"./experiments/wheatrust/OCRNet_cls.yaml",
                        #required=True,
                        type=str)
...
def main():
    ...
    criterion = OhemCrossEntropy(ignore_label=config.TRAIN.IGNORE_LABEL, end_epoch=config.TRAIN.END_EPOCH)
    ...
    for epoch in range(last_epoch, end_epoch):
        ...
        if   (epoch+1) % config.TRAIN.VALID_EPOCH==0:
            valid_loss, mean_IoU, IoU_array, OA = validate(config, epoch, testloader, model, writer_dict, optimizer, results_dict)
            ...
            if args.local_rank <= 0:
                ...
                if mean_IoU > best_mIoU:
                    ...
                    torch.save(model.state_dict(),#model.module.state_dict(),
                            os.path.join(final_output_dir, 'Epoch_{}_best_IOU_{: 4.4f}.pth'.format(epoch, best_mIoU)))
